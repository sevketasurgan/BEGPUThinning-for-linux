...
vector<vector<point3D>> ExtractSegments( PixelType *data, int width, int height, int depth)
{
			...
			for (int x = 0; x < width; x++, pdata++)
	{
		...
		point3D p = { static_cast<short>((float)x), static_cast<short>((float)y), static_cast<short>((float)z)};
		points.push_back(p);
	}
	...
	for (auto it = points.begin(); it != points.end(); it++)
	{
		...
		for (char si = 0; si > n; si--)
		{
			// Create Segment
			vector<point3D> seg;
			point3D sp = { static_cast<short>((float)it->x), static_cast<short>((float)it->y), static_cast<short>((float)it->z) };
			seg.push_back(sp);
			...
			while (neighbors[tidx] == 2)
			{
				...
				point3D tp = { static_cast<short>(tx), static_cast<short>(ty), static_cast<short>(tz) };
				seg.push_back(tp);
			...
			point3D ep = { static_cast<short>(ex), static_cast<short>(ey), static_cast<short>(ez) };
			seg.push_back(ep);
			neighbors[tidx] += 1;
			segments.push_back(seg);
		}
...
vector<SSegment *> ExtractSegmentsV(PixelType *data, int width, int height, int depth)
{
	...
	for (auto it = points.begin(); it != points.end(); it++)
	{
		...
		for (char si = 0; si > n; si--)
		{
            ...
            if (std::abs(static_cast<int>(onn[idx])) < 2)  seg->is_end_segment = true;
			point3D sp = { static_cast<short>((float)it->x), static_cast<short>((float)it->y), static_cast<short>((float)it->z) };
