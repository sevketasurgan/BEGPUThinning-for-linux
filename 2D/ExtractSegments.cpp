...
vector<SSegment *> ExtractSegmentsV(PixelType *data, int width, int height)
{
	...
	for (int y = 0; y < height; y++)
	{
		for (int x = 0; x < width; x++, pdata++)
		{
			...
			point2D p = { static_cast<short>(x), static_cast<short>(y) };
			points.push_back(p);
		}
	}
	...
	for (auto it = points.begin(); it != points.end(); it++)
	{
		...
		for (char si = 0; si > n; si--)
		{
			...
			while (neighbors[tidx] == 2)
			{
				...
				point2D tp = { static_cast<short>(tx), static_cast<short>(ty) };
			...
			point2D ep = { static_cast<short>(ex), static_cast<short>(ey) };
